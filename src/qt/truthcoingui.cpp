...
const QString TruthcoinGUI::DEFAULT_WALLET = "~Default";

TruthcoinGUI::TruthcoinGUI(const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    clientModel(0),
    walletFrame(0),
    ...
    spinnerFrame(0)
{
    ...
    QString windowTitle = tr("Truthcoin Core") + " - ";
#ifdef ENABLE_WALLET
...
void TruthcoinGUI::createActions(const NetworkStyle *networkStyle)
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Truthcoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and truthcoin: URIs)"));
    ...
    aboutAction = new QAction(TextColorIcon(":/icons/about"), tr("&About Truthcoin Core"), this);
    aboutAction->setStatusTip(tr("Show information about Truthcoin Core"));
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for Truthcoin Core"));
    ...
    openAction->setStatusTip(tr("Open a truthcoin: URI or payment request"));

    showHelpMessageAction = new QAction(TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setStatusTip(tr("Show the Truthcoin Core help message to get a list with possible Truthcoin command-line options"));
...
void TruthcoinGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void TruthcoinGUI::createToolBars()
{
    if(walletFrame)
    {
...
void TruthcoinGUI::setClientModel(ClientModel *clientModel)
{
...
#ifdef ENABLE_WALLET
bool TruthcoinGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool TruthcoinGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void TruthcoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void TruthcoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void TruthcoinGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
    trayIcon = new QSystemTrayIcon(this);
    QString toolTip = tr("Truthcoin Core client") + " " + networkStyle->getTitleAddText();
...
void TruthcoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void TruthcoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void TruthcoinGUI::optionsClicked()
{
...
void TruthcoinGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void TruthcoinGUI::showHelpMessageClicked()
{
...
void TruthcoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void TruthcoinGUI::gotoHistoryPage()
{
...
void TruthcoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void TruthcoinGUI::gotoSignMessageTab(QString addr)
{
...
void TruthcoinGUI::setNumConnections(int count)
{
    QString icon;
    switch(count)
    {
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Truthcoin network", "", count));
}

void TruthcoinGUI::setNumBlocks(int count)
{
    if(!clientModel)
        return;
...
void TruthcoinGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Truthcoin"); // default title
...
void TruthcoinGUI::changeEvent(QEvent *e)
{
...
void TruthcoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
#ifdef ENABLE_WALLET
void TruthcoinGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address)
{
    ...
    message((amount)<0 ? tr("Sent transaction") : tr("Incoming transaction"),
             tr("Date: %1\n"
                ...
                "Address: %4\n")
                  .arg(date)
                  .arg(TruthcoinUnits::formatWithUnit(unit, amount, true))
                  .arg(type)
                  .arg(address), CClientUIInterface::MSG_INFORMATION);
}
#endif // ENABLE_WALLET

void TruthcoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void TruthcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool TruthcoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool TruthcoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void TruthcoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void TruthcoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void TruthcoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void TruthcoinGUI::detectShutdown()
{
...
void TruthcoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
static bool ThreadSafeMessageBox(TruthcoinGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void TruthcoinGUI::subscribeToCoreSignals()
{
    // Connect signals to client
    uiInterface.ThreadSafeMessageBox.connect(boost::bind(ThreadSafeMessageBox, this, _1, _2, _3));
}

void TruthcoinGUI::unsubscribeFromCoreSignals()
{
    ...
    menu(0)
{
    ...
    QList<TruthcoinUnits::Unit> units = TruthcoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    foreach (const TruthcoinUnits::Unit unit, units)
    {
        max_width = qMax(max_width, fm.width(TruthcoinUnits::name(unit)));
    }
...
{
    menu = new QMenu();
    foreach(TruthcoinUnits::Unit u, TruthcoinUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(TruthcoinUnits::name(u)), this);
...
{
    setText(TruthcoinUnits::name(newUnits));
}
