...
/*
 * FDLIBM___ieee754_jn(n, x), FDLIBM___ieee754_yn(n, x)
...
#ifdef __STDC__
	double FDLIBM___ieee754_jn(int n, double x)
#else
	double FDLIBM___ieee754_jn(n,x)
	int n; double x;
#endif
{
	int i,hx,ix,lx, sgn;
	...
	if(n==0) return(FDLIBM___ieee754_j0(x));
	if(n==1) return(FDLIBM___ieee754_j1(x));
     ...
     */
		...
		b = invsqrtpi*temp/FDLIBM_sqrt(x);
	        ...
	        a = FDLIBM___ieee754_j0(x);
	        b = FDLIBM___ieee754_j1(x);
	        for(i=1;i<n;i++){
		    temp = b;
     ...
     */
		...
		tmp = n;
		v = two/x;
		tmp = tmp*FDLIBM___ieee754_log(FDLIBM_fabs(v*tmp));
...
#ifdef __STDC__
	double FDLIBM___ieee754_yn(int n, double x) 
#else
	double FDLIBM___ieee754_yn(n,x) 
	int n; double x;
#endif
{
	int i,hx,ix,lx;
	...
	if(n==0) return(FDLIBM___ieee754_y0(x));
	if(n==1) return(sign*FDLIBM___ieee754_y1(x));
     ...
     */
	    ...
	    a = FDLIBM___ieee754_y0(x);
	    b = FDLIBM___ieee754_y1(x);
	/* quit if b is -inf */
