...
#ifdef __STDC__
	double FDLIBM_gamma_r(double x, int *signgamp) /* wrapper lgamma_r */
#else
	double FDLIBM_gamma_r(x,signgamp)              /* wrapper lgamma_r */
        double x; int *signgamp;
#endif
{
#ifdef _IEEE_LIBM
	return FDLIBM___ieee754_gamma_r(x,signgamp);
#else
        double y;
        y = FDLIBM___ieee754_gamma_r(x,signgamp);
