...
#ifdef __STDC__
	double FDLIBM___ieee754_sqrt(double x)
#else
	double FDLIBM___ieee754_sqrt(x)
	double x;
#endif
{
	double z;
...
A.  FDLIBM_sqrt(x) by Newton Iteration
   ...
   (1)	Initial approximation
	...
	follows.
		...
		k  := (x0>>1) + 0x1ff80000;
		y0 := k - T1[31&(k>>15)].	... y ~ FDLIBM_sqrt(x) to 8 bits
...
B.  FDLIBM_sqrt(x) by Reciproot Iteration
   ...
   (1)	Initial approximation
	...
	we obtain a 7.8-bit approximation of 1/FDLIBM_sqrt(x) as follows.
	    ...
	    k := 0x5fe80000 - (x0>>1);
	    y0:= k - T2[63&(k>>14)].	... y ~ 1/FDLIBM_sqrt(x) to 7.8 bits

	Here k is a 32-bit integer and T2[] is an integer array 
	containing correction terms. Now magically the floating
	value of y (y's leading 32-bit word is y0, the value of
	its trailing word y1 is set to zero) approximates 1/FDLIBM_sqrt(x)
	to almost 7.8-bit.
